...
<style>
    ...
    * {
    ...
    /* Selectbox Dropdown Box and Popover Menu Styling:
       Clear White background with Slate (#1e293b) text and Times New Roman font
    */
    ...
    /* File upload container (Month/Report area beneath Drop header): Pure White background under all states */
    div[data-testid="stFileUploader"],
    div[data-testid="stFileUploader"] > section,
    div[data-testid="stFileUploader"] > section > div,
    div[data-testid="stFileUploader"] [data-testid="stFileDropzone"],
    div[data-testid="stFileDropzone"] {
        background-color: #FFFFFF !important; /* Force pure White under all states */
        background: #FFFFFF !important;
        border-radius: 6px !important;
    }
    ...
    /* Force all text, secondary labels, and helper descriptions inside the file upload container to be sharp Dark Charcoal/Black (#000000) */
    div[data-testid="stFileUploader"] *,
    ...
    /* Clean File Uploader Button styling (not the delete button): standard background Light Gray (#EFEFEF) with black text, hover Gray (#D0D0D0) */
    div[data-testid="stFileUploader"] button:not([data-testid="stFileUploaderDeleteBtn"]):not([aria-label*="Delete"]):not([aria-label*="Remove"]):not([class*="delete"]) {
        ...
        font-size: 0px !important; /* Hide original text to avoid duplicate */
    ...
    /* Hide all native nested elements inside the upload button so their text doesn't show */
    div[data-testid="stFileUploader"] button:not([data-testid="stFileUploaderDeleteBtn"]):not([aria-label*="Delete"]):not([aria-label*="Remove"]):not([class*="delete"]) * {
    ...
    /* Inject exactly one single "Upload" text using the ::after pseudo-element */
    ...
    /* Distinct styling for the File Deletion / Cancellation button */
    ...
    div[data-testid="stFileUploader"] button[class*="delete"] {
        background-color: #F8D7DA !important; /* Soft Red/Pink background for clarity */
    ...
    /* Hide all native nested elements inside the delete button */
    div[data-testid="stFileUploader"] button[data-testid="stFileUploaderDeleteBtn"] *,
    div[data-testid="stFileUploader"] button[aria-label*="Delete"] *,
    div[data-testid="stFileUploader"] button[aria-label*="Remove"] *,
    div[data-testid="stFileUploader"] button[class*="delete"] * {
    ...
    /* Inject "🗑️ Remove" text with clear color */
    ...
    div[data-testid="stFileUploader"] button[class*="delete"]:hover {
        background-color: #F1AEB5 !important; /* Deeper Red hover background */
    ...
    /* Uniform styling for all executive buttons and contact form button */
    .stButton > button, .stDownloadButton > button, .feedback-submit {
        background-color: #1F4E78 !important; /* Deep Royal Navy Blue */
        color: #FFFFFF !important; /* White text */
        border: 1.5px solid #D4AF37 !important; /* Premium Gold border */
        font-weight: bold !important;
        font-family: 'Times New Roman', Times, Georgia, serif !important;
    ...
    .stButton > button:hover, .stDownloadButton > button:hover, .feedback-submit:hover {
        ...
        color: #1F4E78 !important; /* Navy text on hover */
        border-color: #1F4E78 !important;
        box-shadow: 0 6px 12px rgba(212, 175, 55, 0.4) !important;
    }
    
    /* Compact Centered Feedback Support Form container */
    .feedback-box {
        background-color: #FFFFFF !important;
    ...
    /* Centered Premium Signature Footer */
    .executive-footer {
        text-align: center !important;
        padding: 25px 10px !important;
        color: #1F4E78 !important; /* Navy Blue text */
        font-family: 'Times New Roman', Times, Georgia, serif !important;
    ...
    /* Style st.tabs headers to strictly be #1e293b and Times New Roman */
    .stTabs [data-baseweb="tab"], .stTabs button, .stTabs [role="tab"], .stTabs [data-baseweb="tab"] *, .stTabs button *, .stTabs [role="tab"] * {
        color: #1e293b !important;
        font-family: 'Times New Roman', Times, Georgia, serif !important;
    }
    
    /* High contrast table styles (standard tables, st.table, and HTML elements) */
    ...
    /* Streamlit Interactive Dataframe/Data Editor Styling */
    ...
    /* Ensure any DOM-based headers/cells within dataframes render cleanly with black text and gray borders */
    ...
    /* Target any editable text area, inputs, values, or metrics within table containers */
    ...
    /* High contrast interactive toolbar & download buttons styling within dataframes */
    div[data-testid="stDataFrame"] button,
    div[data-testid="stDataFrameInteractive"] button,
    div[data-testid="stDataFrame"] [role="button"],
    div[data-testid="stDataFrameInteractive"] [role="button"] {
        background-color: #FFFFFF !important;
        color: #1e293b !important;
        border: 1px solid #CCCCCC !important;
        border-radius: 4px !important;
    ...
    div[data-testid="stDataFrameInteractive"] button:hover,
    div[data-testid="stDataFrame"] [role="button"]:hover,
    div[data-testid="stDataFrameInteractive"] [role="button"]:hover {
        background-color: #F1F5F9 !important;
        color: #000000 !important;
    ...
    div[data-testid="stDataFrameInteractive"] svg {
        stroke: #1e293b !important;
        fill: #1e293b !important;
    }
    
    div[data-testid="stDataFrame"] button:hover svg,
    div[data-testid="stDataFrameInteractive"] button:hover svg {
...
# --- 1. CONFIGURATION LAYOUT (SIDE-BY-SIDE MONTHLY PANEL) ---
...
for idx in range(3):
    ...
    with [col1, col2, col3][idx]:
        ...
        # Simplified Date Picker: Month/Year Dropdown selector logic (e.g., "May 2026")
        available_dates = []
        ...
        # Get the current value from the session state and find its index
        ...
        if file_val is not None:
            if m_data["file_name"] != file_val.name:
                m_data["file_name"] = file_val.name
                m_data["file_bytes"] = file_val.getvalue()
                m_data["is_uploaded"] = True
                parse_monthly_file(m_data["file_bytes"], idx)
                
        # Connection status feedback in high-contrast labels
...
with btn_col2:
    ...
    # High-Fidelity Quick Evaluation load button
    ...
    if load_mock:
        ...
        for idx in range(3):
            ...
            for clinic in CLINICS_METADATA:
                ...
                # Fetch rejected cases count to seed the rejections
...
if st.session_state["data_processed"]:
    ...
    # Strategic High-Contrast Metrics Row
    ...
    # Elegant Tab Workspace
    tab_graphs, tab_editor = st.tabs(["📊 Performance Graphics", "✏️ Comprehensive Clinic Ledgers"])
    ...
    def generate_executive_spreadsheet():
        ...
        # Right rejections summary table
        ...
        # Balance Reconciliation verification block
        ...
        # Draw elegant double gold lines
        ...
        # Adjust Column dimensions for Summary Page
        ...
        for clinic in CLINICS_METADATA:
            ...
            for m_idx in range(3):
                ...
                # Monthly banner separator with gold bottom
                ...
                row_stack += 3 # Generous spacing before the next month's card
                
            # Adjust column width for this clinic sheet
...
# --- 3. RELOCATED SUPPORT & FEEDBACK FORM ---
st.markdown("""
...
# --- 4. PREMIUM EXECUTIVE SIGNATURE FOOTER ---
st.markdown("""
